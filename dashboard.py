...
<body>
  ...
  <script>
    ...
    function renderTable(){const rows=filteredRows();const body=byId("tableBody");const markup=rows.map(row=>rowMarkup(row)).join("");const copy=rows.map(row=>rowMarkup(row,true)).join("");body.innerHTML=rows.length>8?markup+copy:markup;startSellerAutoScroll(rows.length>8)}
    function startSellerAutoScroll(enabled){const wrap=document.querySelector(".table-wrap");const body=byId("tableBody");if(sellerScrollFrame){cancelAnimationFrame(sellerScrollFrame);sellerScrollFrame=null}wrap.scrollTop=0;if(!enabled||window.matchMedia("(max-width: 640px)").matches)return;let last=null;const speed=12;const tick=time=>{if(last===null)last=time;const elapsed=time-last;last=time;const loopHeight=body.scrollHeight/2;if(loopHeight>wrap.clientHeight){wrap.scrollTop+=(speed*elapsed)/1000;if(wrap.scrollTop>=loopHeight)wrap.scrollTop=0}sellerScrollFrame=requestAnimationFrame(tick)};sellerScrollFrame=requestAnimationFrame(tick)}
