...
EXCEL_PATH = BASE_DIR / "base_vendas.xlsx"
...
<body>
  ...
  <script>
    ...
    function renderTable(){const rows=filteredRows();const body=byId("tableBody");const markup=rows.map(row=>rowMarkup(row)).join("");const duplicate=rows.map(row=>rowMarkup(row,true)).join("");body.innerHTML=rows.length>8?markup+duplicate:markup;body.classList.toggle("auto-scroll",rows.length>8)}
...
def read_excel_dashboard_data():
    ...
    existing = {item["name"] for item in weeks}
    default_goal = weeks[0]["goal"] if weeks else 0
    for index in range(1, 5):
        name = f"Semana {index}"
        if name not in existing:
            weeks.append({"name": name, "goal": default_goal, "reached": 0.0, "missing": default_goal, "percent": 0.0})
    weeks = sorted(weeks, key=week_sort_key)[:5]
...
def read_dashboard_data():
    if EXCEL_PATH.exists():
        return read_excel_dashboard_data()
