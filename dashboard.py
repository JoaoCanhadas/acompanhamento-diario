...
function normalizeWeeks(data){if(Array.isArray(data.weeks)&&data.weeks.length)return data.weeks;const summary=data.summary||{};const goal=Number(summary.weekGoal||700000);return[1,2,3,4,5].map(week=>({name:`Semana ${week}`,goal,reached:week===1?Number(summary.weekRevenue||0):0,missing:week===1?Number(summary.weekMissing||goal):goal,percent:week===1?Number(summary.weekPercent||0):0}))}
...
function rowMarkup(row,copy=false){return`<tr${copy?' class="loop-copy"':''}><td class="seller">${row.seller}</td><td>${formatMoney(row.commitment)}</td><td>${formatMoney(row.reached)}</td><td class="money ${moneyClass(row.missing)}">${formatMoney(row.missing)}</td><td>${row.percent}%</td><td><span class="tag ${row.status}">${row.status==="ok"?"Superou":"Falta"}</span></td></tr>`}
function renderTable(){const rows=filteredRows();const markup=rows.map(row=>rowMarkup(row)).join("");const duplicate=rows.map(row=>rowMarkup(row,true)).join("");const body=byId("tableBody");body.innerHTML=rows.length>8?markup+duplicate:markup;body.classList.toggle("auto-scroll",rows.length>8)}
...
def read_weekly_goals():
    ...
    if WEEKLY_GOALS_PATH.exists():
        try:
            stored = json.loads(WEEKLY_GOALS_PATH.read_text(encoding="utf-8"))
...
def save_weekly_goals(goals):
    normalized = default_weekly_goals()
    for name, value in goals.items():
        normalized[normalize_week_name(name)] = money(value)
    WEEKLY_GOALS_PATH.write_text(json.dumps(normalized, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return normalized
...
def apply_weekly_goals(data):
    ...
    for index in range(1, 6):
        name = f"Semana {index}"
        if not any(normalize_week_name(item.get("name")) == name for item in weeks):
            weeks.append({"name": name, "goal": 0, "reached": 0, "missing": 0, "percent": 0})
    for week in weeks:
        name = normalize_week_name(week.get("name"))
        week["name"] = name
        week["goal"] = money(goals.get(name, DEFAULT_WEEKLY_GOAL))
    ...
    current = data["weeks"][0] if data["weeks"] else {}
    summary["weekGoal"] = current.get("goal", 0)
    summary["weekRevenue"] = current.get("reached", 0)
    summary["weekMissing"] = current.get("missing", 0)
    summary["weekPercent"] = current.get("percent", 0)
    return data
...
def read_weekly_blocks(workbook):
    ...
    for row_number in range(1, sheet.max_row + 1):
        ...
        percent = (week_reached / week_goal * 100) if week_goal else 0
        weeks.append({"name": week_name.title(), "goal": week_goal, "reached": week_reached, "missing": money(week_goal - week_reached), "percent": round(percent, 1)})
...
def select_daily_total(daily_totals):
    ...
    for item in daily_totals:
        if item["date"] == today:
            return item
    dated = [item for item in daily_totals if item["date"] and item["date"] <= today]
    return max(dated, key=lambda item: item["date"]) if dated else daily_totals[-1]
...
def read_compromisso_maio_data():
    ...
    try:
        ...
        workbook.close()
    daily = select_daily_total(daily_totals)
    total_commitment = money(daily["commitment"] if daily else sum(item["commitment"] for item in rows))
    total_reached = money(daily["reached"] if daily else sum(item["reached"] for item in rows))
    total_missing = money(daily["missing"] if daily else total_commitment - total_reached)
...
def read_legacy_excel_dashboard_data():
    ...
    try:
        ...
        rows_by_seller, weeks_by_name = {}, {}
        for row in sheet.iter_rows(min_row=2, values_only=True):
            _, week, seller, goal, reached, *_ = list(row) + [None] * 2
            if not seller:
                continue
            ...
            seller_row["commitment"] = money(seller_row["commitment"] + money(goal))
            seller_row["reached"] = money(seller_row["reached"] + money(reached))
            week_row = weeks_by_name.setdefault(week_name, {"name": week_name, "goal": 0.0, "reached": 0.0})
            week_row["goal"] = money(week_row["goal"] + money(goal))
            week_row["reached"] = money(week_row["reached"] + money(reached))
    finally:
        workbook.close()
    rows = []
